...
set service_URL=!API_URL!/service/validate-compu-methods?
...
rem ==== Output Files ==== 
set LogFile=%outputPath%\CM_LOG.log
set tempFile=%outputPath%\tmp.txt
set outFile=%outputPath%\CM_RESULTS.zip
goto :MAIN
...
echo "::::::COMPUMETHODS_SERVICE_WEB_APP_DEMO:::::">!LogFile!
...
curl -X POST %service_URL%access-id=%ACCESS_ID% -H "Content-type:application/json" -d "{\"xlFileID\":\"!exl_FileID!\",\"a2lFileID\":\"!a2l_FileID!\"}">!tempFile!
for /f "tokens=1,2 delims=:{}, " %%x in (!tempFile!) do (
	if %%~x==FileID (
...
echo Results saved to file.>>!LogFile!
